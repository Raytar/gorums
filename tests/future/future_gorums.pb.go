...
// FutureQC1 asynchronously invokes a quorum call on configuration c
// and returns a FutureResponse1, which can be used to inspect the quorum call
// reply and error when available.
func (c *Configuration) FutureQC1(ctx context.Context, in *Request1) *FutureResponse1 {
	fut := &FutureResponse1{
	...
	go c.futureQC1Recv(ctx, in, msgID, expected, replyChan, fut)

	return fut
}

func (c *Configuration) futureQC1Recv(ctx context.Context, in *Request1, msgID uint64, expected int, replyChan chan *orderingResult, fut *FutureResponse1) {
	defer close(fut.c)
	...
	var (
		replyValues = make([]*Response1, 0, c.n)
		reply       *Response1
		errs        []GRPCError
		quorum      bool
	)
	...
	for {
		...
		case r := <-replyChan:
			...
			if reply, quorum = c.qspec.FutureQC1QF(in, replyValues); quorum {
				fut.Response1, fut.err = reply, nil
				return
			}
		case <-ctx.Done():
			fut.Response1, fut.err = reply, QuorumCallError{ctx.Err().Error(), len(replyValues), errs}
			return
		}
		if len(errs)+len(replyValues) == expected {
			fut.Response1, fut.err = reply, QuorumCallError{"incomplete call", len(replyValues), errs}
			return
		}
	}
}

// FutureQC1Handler is the server API for the FutureQC1 rpc.
type FutureQC1Handler interface {
	FutureQC1(*Request1) *Response1
}

// RegisterFutureQC1Handler sets the handler for FutureQC1.
func (s *GorumsServer) RegisterFutureQC1Handler(handler FutureQC1Handler) {
	s.srv.registerHandler(futureQC1MethodID, func(in *ordering.Message) *ordering.Message {
		req := new(Request1)
		...
		resp := handler.FutureQC1(req)
		data, err := proto.MarshalOptions{AllowPartial: true, Deterministic: true}.Marshal(resp)
		if err != nil {
			return new(ordering.Message)
		}
		return &ordering.Message{Data: data, MethodID: futureQC1MethodID}
	})
}

// FutureQC2 asynchronously invokes a quorum call on configuration c
// and returns a FutureResponse2, which can be used to inspect the quorum call
// reply and error when available.
func (c *Configuration) FutureQC2(ctx context.Context, in *Request2) *FutureResponse2 {
	fut := &FutureResponse2{
		NodeIDs: make([]uint32, 0, c.n),
		c:       make(chan struct{}, 1),
	}
	...
	go c.futureQC2Recv(ctx, in, msgID, expected, replyChan, fut)

	return fut
}

func (c *Configuration) futureQC2Recv(ctx context.Context, in *Request2, msgID uint64, expected int, replyChan chan *orderingResult, fut *FutureResponse2) {
	defer close(fut.c)
	...
	var (
		replyValues = make([]*Response2, 0, c.n)
		reply       *Response2
		errs        []GRPCError
		quorum      bool
	)
	...
	for {
		...
		case r := <-replyChan:
			...
			replyValues = append(replyValues, data)
			if reply, quorum = c.qspec.FutureQC2QF(in, replyValues); quorum {
				fut.Response2, fut.err = reply, nil
				return
			}
		case <-ctx.Done():
			fut.Response2, fut.err = reply, QuorumCallError{ctx.Err().Error(), len(replyValues), errs}
			return
		}
		if len(errs)+len(replyValues) == expected {
			fut.Response2, fut.err = reply, QuorumCallError{"incomplete call", len(replyValues), errs}
			return
		}
	}
}

// FutureQC2Handler is the server API for the FutureQC2 rpc.
type FutureQC2Handler interface {
	FutureQC2(*Request2) *Response2
}

// RegisterFutureQC2Handler sets the handler for FutureQC2.
func (s *GorumsServer) RegisterFutureQC2Handler(handler FutureQC2Handler) {
	s.srv.registerHandler(futureQC2MethodID, func(in *ordering.Message) *ordering.Message {
		req := new(Request2)
		err := proto.UnmarshalOptions{AllowPartial: true, DiscardUnknown: true}.Unmarshal(in.GetData(), req)
		// TODO: how to handle marshaling errors here
		if err != nil {
		...
		resp := handler.FutureQC2(req)
		data, err := proto.MarshalOptions{AllowPartial: true, Deterministic: true}.Marshal(resp)
		if err != nil {
			return new(ordering.Message)
		}
		return &ordering.Message{Data: data, MethodID: futureQC2MethodID}
	})
}

// QuorumSpec is the interface of quorum functions for FutureTest.
type QuorumSpec interface {

	// FutureQC1QF is the quorum function for the FutureQC1
	// asynchronous ordered quorum call method. The in parameter is the request object
	// supplied to the FutureQC1 method at call time, and may or may not
	// be used by the quorum function. If the in parameter is not needed
	// you should implement your quorum function with '_ *Request1'.
	FutureQC1QF(in *Request1, replies []*Response1) (*Response1, bool)

	// FutureQC2QF is the quorum function for the FutureQC2
	// asynchronous ordered quorum call method. The in parameter is the request object
	// supplied to the FutureQC2 method at call time, and may or may not
	// be used by the quorum function. If the in parameter is not needed
	// you should implement your quorum function with '_ *Request2'.
	FutureQC2QF(in *Request2, replies []*Response2) (*Response2, bool)
}

const hasOrderingMethods = true

const futureQC1MethodID int32 = 0
const futureQC2MethodID int32 = 1
...
type internalResponse1 struct {
	nid   uint32
	reply *Response1
	err   error
}

type internalResponse2 struct {
	nid   uint32
	reply *Response2
	err   error
}

// FutureResponse1 is a future object for processing replies.
type FutureResponse1 struct {
	// the actual reply
	*Response1
	NodeIDs []uint32
	err     error
...
func (f *FutureResponse1) getc() chan struct{} {
	return f.c
}

// Get returns the reply and any error associated with the called method.
// The method blocks until a reply or error is available.
func (f *FutureResponse1) Get() (*Response1, error) {
	<-f.c
	return f.Response1, f.err
}

// Done reports if a reply and/or error is available for the called method.
func (f *FutureResponse1) Done() bool {
	select {
	case <-f.c:
		return true
	default:
...
// FutureResponse2 is a future object for processing replies.
type FutureResponse2 struct {
	// the actual reply
	*Response2
	NodeIDs []uint32
	err     error
...
func (f *FutureResponse2) getc() chan struct{} {
	return f.c
}

// Get returns the reply and any error associated with the called method.
// The method blocks until a reply or error is available.
func (f *FutureResponse2) Get() (*Response2, error) {
	<-f.c
	return f.Response2, f.err
}

// Done reports if a reply and/or error is available for the called method.
func (f *FutureResponse2) Done() bool {
	select {
	case <-f.c:
		return true
	default:
